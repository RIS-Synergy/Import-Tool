...
*   **`Registry`**: Fetches and manages **API endpoints** (e.g., for funding and projects) from a central registry (`https://api.forschungsdaten.at/registry.json`). It stores these endpoints and provides methods to retrieve specific URLs, which are then used by `FundingAgency` and `Project` for external API interactions.
