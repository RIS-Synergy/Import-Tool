...
export async function uploadProjectApplicationClusters(project, template, ris, settings) {
  ...
  if (template.applicationId) {
    project = await handleCluster(
      applicationClusters, template.applicationId, "ApplicationCluster", "/applications", project, ris, settings);
  }
  if (template.awardId) {
    project = await handleCluster(
      awardClusters, template.awardId, "AwardCluster", "/awards", project, ris, settings);
  }
