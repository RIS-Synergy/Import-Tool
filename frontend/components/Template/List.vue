    ...
    <v-row>
      <v-col cols="6" v-for="item in data" :key="item.id">
        <v-card>
          ...
          <v-card-actions>
            <v-btn @click="enableEditDialog(item)"> Edit </v-btn>
            <v-spacer></v-spacer>
            <v-btn variant="tonal" :to="`${templateType}/${item.id}`">
              View
            </v-btn>
          </v-card-actions>
        </v-card>
...
const newTemplateTitle = computed(() => {
  const newOrUpdate = store.templateId ? "Update" : "New";
  const firstLetterCapitalized =
    props.templateType.charAt(0).toUpperCase() + props.templateType.slice(1);
